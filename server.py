...
def switches_usados_sala(sala_id):
    ...
    for switch_id, switch_nome, switch_marca, switch_modelo in switches_rows:
        ...
        for porta_row in cur.fetchall():
            ...
            # Verificar se a porta está mapeada para patch panel
            cur.execute('''
                SELECT pp.nome, ppp.numero_porta, ppp.equipamento_id, pp.prefixo_keystone
            ...
            if patch_panel_row:
                ...
                patch_panel_nome, porta_patch_panel, equipamento_id, prefixo_keystone = patch_panel_row
...
def atualizar_cabo(id):
    ...
    try:
        ...
        conn.commit()
        registrar_log(session.get('username'), 'atualizar_cabo', f'Cabo ID {id} atualizado', 'sucesso', db_file)
        return jsonify({'status': 'ok'})
...
def excluir_cabo(id):
    ...
    registrar_log(session.get('username'), 'excluir_cabo', f'Cabo ID {id} excluído', 'sucesso', db_file)
    return jsonify({'status': 'ok'})
...
def tipos_cabos():
    try:
        # Carregar tipos de cabos do arquivo JSON centralizado
        json_path = os.path.join('static', 'data', 'tipos-cabos.json')
        
        if not os.path.exists(json_path):
            return jsonify({'erro': 'Arquivo de tipos de cabos não encontrado'}), 404
        ...
        return jsonify({'erro': f'Erro ao carregar tipos de cabos: {str(e)}'}), 500
...
def listar_conexoes_cabos():
    ...
    if cabo_id:
        ...
        # Buscar conexões de um cabo específico (apenas ativas)
        cur.execute('''
            SELECT cc.id, cc.cabo_id, cc.equipamento_origem_id, cc.equipamento_destino_id,
                   cc.porta_origem, cc.porta_destino, cc.sala_id, cc.observacao,
                   cc.data_conexao, cc.data_desconexao,
                   c.codigo_unico, c.tipo, c.comprimento, c.marca, c.modelo,
                   eo.nome as equipamento_origem_nome,
                   CASE 
                       WHEN ed.tipo = 'patch_panel' THEN 
                           CASE 
                               WHEN cc.observacao LIKE '%Conexão via Patch Panel%' THEN ed.nome
                               ELSE COALESCE(
                                   CASE WHEN ppp_equip.sala_id = cc.sala_id THEN ppp_equip.nome END,
                                   ed.nome
                               )
                           END
                       ELSE 
                           COALESCE(pp2.nome, ed.nome)
                   END as equipamento_destino_nome,
                   s.nome as sala_nome,
                   CASE WHEN ed.tipo = 'patch_panel' OR cc.observacao LIKE '%Conexão via Patch Panel%' THEN 1 ELSE 0 END as via_patch_panel,
                   CASE WHEN ed.tipo = 'patch_panel' THEN ed.nome END as patch_panel_nome,
                   cc.porta_destino as patch_panel_porta
            FROM conexoes_cabos cc
            JOIN cabos c ON cc.cabo_id = c.id
            LEFT JOIN equipamentos eo ON cc.equipamento_origem_id = eo.id
            LEFT JOIN equipamentos ed ON cc.equipamento_destino_id = ed.id
            LEFT JOIN salas s ON cc.sala_id = s.id
            LEFT JOIN patch_panel_portas ppp ON ed.id = ppp.patch_panel_id AND cc.porta_destino = ppp.numero_porta
            LEFT JOIN equipamentos ppp_equip ON ppp.equipamento_id = ppp_equip.id
            LEFT JOIN patch_panel_portas ppp2 ON cc.porta_destino IS NOT NULL AND ppp2.equipamento_id = ed.id AND ppp2.numero_porta = cc.porta_destino
            LEFT JOIN equipamentos pp2 ON pp2.id = ppp2.patch_panel_id AND pp2.tipo = 'patch_panel'
            WHERE cc.cabo_id = ? 
            AND cc.data_desconexao IS NULL
            AND cc.equipamento_origem_id IS NOT NULL
            AND cc.equipamento_destino_id IS NOT NULL
            GROUP BY cc.equipamento_origem_id, cc.equipamento_destino_id, cc.porta_origem, cc.porta_destino
            ORDER BY cc.data_conexao DESC
        ''', (cabo_id,))
    else:
...
@tecnico_required
def atualizar_conexao_cabo(id):
    db_file = session.get('db')
    if not db_file:
        return jsonify({'erro': 'Nenhuma empresa selecionada!'}), 400
    
    data = request.get_json()
    equipamento_origem_id = data.get('equipamento_origem_id')
    equipamento_destino_id = data.get('equipamento_destino_id')
    porta_origem = data.get('porta_origem')
    ...
    if update_fields:
        ...
        if cur.rowcount == 0:
            conn.close()
            return jsonify({'status': 'erro', 'mensagem': 'Erro ao atualizar conexão'}), 500
...
@app.route('/conexoes-cabos/<int:id>/reconectar', methods=['POST'])
@tecnico_required
def reconectar_cabo(id):
    db_file = session.get('db')
    if not db_file:
        return jsonify({'erro': 'Nenhuma empresa selecionada!'}), 400
    ...
    # Verificar se a conexão existe e está desconectada
    cur.execute('SELECT id, data_desconexao FROM conexoes_cabos WHERE id = ?', (id,))
    conexao = cur.fetchone()
    if not conexao:
        conn.close()
        return jsonify({'status': 'erro', 'mensagem': 'Conexão não encontrada'}), 404
    
    if not conexao[1]:  # Se data_desconexao é NULL, já está conectada
        conn.close()
        return jsonify({'status': 'erro', 'mensagem': 'Conexão já está ativa'}), 400
    
    # Reconectar a conexão
    cur.execute('UPDATE conexoes_cabos SET data_desconexao = NULL WHERE id = ?', (id,))
    if cur.rowcount == 0:
        conn.close()
        return jsonify({'status': 'erro', 'mensagem': 'Erro ao reconectar cabo'}), 500
    
    conn.commit()
    conn.close()
    
    registrar_log(session.get('username'), 'reconectar_cabo', f'Conexão de cabo {id} reconectada', 'sucesso', db_file)
    return jsonify({'status': 'ok', 'mensagem': 'Cabo reconectado com sucesso'})
...
def logs_por_sala(sala_id):
    try:
        # Usar o arquivo de log da empresa atual
        db_file = session.get('db')
        if not db_file:
            return jsonify({'status': 'erro', 'mensagem': 'Nenhuma empresa selecionada!'}), 400
        ...
        # Filtra logs que mencionam a sala pelo ID ou nome
        sala_id_str = f"ID={sala_id}"
        sala_id_alt = f"sala_id={sala_id}"
        sala_id_alt2 = f"Sala ID: {sala_id}"
        sala_id_alt3 = f"sala {sala_id}"
        sala_id_alt4 = f"Sala {sala_id}"
        
        # Buscar logs que contenham qualquer uma dessas referências
        logs_sala = []
        for log in logs:
            if any(ref in log for ref in [sala_id_str, sala_id_alt, sala_id_alt2, sala_id_alt3, sala_id_alt4]):
                logs_sala.append(log)
...
def log_exportacao():
    ...
    acao = dados.get('acao', 'EXPORTACAO_DESCONHECIDA')
    detalhes = dados.get('detalhes', 'Sem detalhes')
    status = dados.get('status', 'sucesso')
    
    # Registrar log de exportação na empresa atual
...
def marcar_cabo_defeito(id):
    ...
    conn.commit()
    conn.close()
    registrar_log(session.get('username'), 'marcar_cabo_defeito', f'Cabo ID {id} marcado como defeito', 'sucesso', db_file)
    return jsonify({'status': 'ok'})
...
def reparar_cabo(id):
    ...
    conn.commit()
    conn.close()
    registrar_log(session.get('username'), 'reparar_cabo', f'Cabo ID {id} reparado - Status: {novo_status}', 'sucesso', db_file)
    return jsonify({'status': 'ok'})
...
def criar_patch_panel():
    ...
    if _is_json_mode(db_file):
        ...
        # duplicidade por nome/andar
        if any((pp.get('andar') == dados.get('andar') and (pp.get('nome') or '').lower() == (dados.get('nome') or '').lower()) for pp in patch_panels):
            return jsonify({'erro': f'Já existe um patch panel com o nome "{dados.get("nome")}" no {dados.get("andar")}º andar!'}), 400
        andar_base = 20 + int(dados.get('andar') or 0)
        # código sequencial por andar
        count = sum(1 for pp in patch_panels if pp.get('andar') == dados.get('andar'))
        codigo = f"PP{andar_base}-{(count + 1):02d}"
        patch_panel_id = _json_next_id(patch_panels)
        registro = {
            'id': patch_panel_id,
            'codigo': codigo,
            'nome': dados.get('nome'),
            'andar': dados.get('andar'),
            'prefixo_keystone': dados.get('prefixo_keystone'),
            'porta_inicial': 1,
            'num_portas': int(dados.get('num_portas', 500)),
            'status': dados.get('status', 'ativo'),
            'descricao': dados.get('descricao'),
            'data_criacao': datetime.now().isoformat()
        }
        patch_panels.append(registro)
        _json_write_table(db_file, 'patch_panels', patch_panels)
        # criar equipamento correspondente
        equipamentos = _json_read_table(db_file, 'equipamentos')
        equipamento_patch_panel_id = _json_next_id(equipamentos)
        equipamentos.append({
            'id': equipamento_patch_panel_id,
            'nome': dados.get('nome'),
            'tipo': 'patch_panel',
            'marca': 'Patch Panel',
            'modelo': 'Patch Panel',
            'descricao': dados.get('descricao') or f"Patch Panel {dados.get('nome')}",
            'sala_id': None,
            'status': 'ativo',
            'dados': {}
        })
        ...
        for i in range(int(dados.get('num_portas', 500))):
            portas.append({
                'id': _json_next_id(portas),
        ...
        detalhes = f"Patch Panel criado: {dados.get('nome')} - {int(dados.get('num_portas', 500))} portas (1-{int(dados.get('num_portas', 500))})"
        registrar_log(session.get('username', 'desconhecido'), 'CRIAR_PATCH_PANEL', detalhes, 'sucesso', db_file)
        return jsonify({'status': 'ok', 'patch_panel_id': patch_panel_id, 'equipamento_id': equipamento_patch_panel_id})
    else:
        conn = sqlite3.connect(db_file)
        cur = conn.cursor()
        ...
        try:
            ...
            # Criar equipamento correspondente ao patch panel
            cur.execute('''
                INSERT INTO equipamentos (nome, tipo, marca, modelo, descricao, sala_id, status)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (dados['nome'], 'patch_panel', 'Patch Panel', 'Patch Panel', dados.get('descricao', f'Patch Panel {dados["nome"]}'), None, 'ativo'))
            
            equipamento_patch_panel_id = cur.lastrowid
            ...
            registrar_log(session.get('username', 'desconhecido'), 'CRIAR_PATCH_PANEL', detalhes, 'erro', db_file)
            return jsonify({'status': 'erro', 'mensagem': f'Erro ao criar patch panel: {str(e)}'}), 500
        
        finally:
            conn.close()

@app.route('/patch-panels/migrar', methods=['POST'])
@admin_required
def migrar_patch_panels():
    """Migrar patch panels existentes para criar equipamentos correspondentes"""
    db_file = session.get('db')
    if not db_file:
        return jsonify({'erro': 'Nenhuma empresa selecionada!'}), 400
...
def listar_patch_panels():
    ...
    if _is_json_mode(db_file):
        ...
        resultado = []
        for pp in pps:
            andar = int(pp.get('andar') or 0)
            andar_nome = f"{andar}º Andar" if andar > 0 else "Térreo"
        ...
        resultado.sort(key=lambda x: (x.get('andar') or 0, x.get('nome') or ''))
        return jsonify(resultado)
    else:
        conn = sqlite3.connect(db_file)
        cur = conn.cursor()
...
def validar_portas_patch_panel():
    ...
    if _is_json_mode(db_file):
        ...
        return jsonify({'status': 'ok', 'portas_existentes': portas_existentes})
    else:
        conn = sqlite3.connect(db_file)
        cur = conn.cursor()
...
def listar_patch_panels_por_andar(andar):
    ...
    if _is_json_mode(db_file):
        pps = [pp for pp in _json_read_table(db_file, 'patch_panels') if int(pp.get('andar') or 0) == int(andar)]
        portas = _json_read_table(db_file, 'patch_panel_portas')
        resultado = []
        for pp in pps:
            portas_do_pp = [p for p in portas if p.get('patch_panel_id') == pp.get('id')]
            ocupadas = sum(1 for p in portas_do_pp if p.get('equipamento_id') is not None or p.get('switch_id') is not None)
            total = int(pp.get('num_portas') or 0)
            andar_nome = f"{andar}º Andar" if int(andar) > 0 else "Térreo"
            porta_inicial = int(pp.get('porta_inicial') or 1)
            porta_final = porta_inicial + total - 1 if total else porta_inicial
            resultado.append({
                'id': pp.get('id'),
                'codigo': pp.get('codigo'),
                'nome': pp.get('nome'),
        ...
        try:
            cur.execute('''
                SELECT pp.id, pp.codigo, pp.nome, pp.andar, pp.porta_inicial, pp.num_portas, pp.status, pp.descricao, pp.data_criacao,
                       (SELECT COUNT(*) FROM patch_panel_portas ppp WHERE ppp.patch_panel_id = pp.id AND (ppp.equipamento_id IS NOT NULL OR ppp.switch_id IS NOT NULL)) as portas_ocupadas
                FROM patch_panels pp
                WHERE pp.andar = ?
                ORDER BY pp.nome
            ''', (andar,))
...
def atualizar_patch_panel(id):
    ...
    if _is_json_mode(db_file):
        ...
        # duplicidade
        if any(x.get('id') != id and x.get('andar') == dados.get('andar') and (x.get('nome') or '').lower() == (dados.get('nome') or '').lower() for x in pps):
            return jsonify({'erro': f'Já existe um patch panel com o nome "{dados.get("nome")}" no {dados.get("andar")}º andar!'}), 400
        porta_inicial_atual = int(pp.get('porta_inicial') or 1)
        num_portas_atual = int(pp.get('num_portas') or 0)
        porta_inicial_nova = int(dados.get('porta_inicial', 1))
        num_portas_nova = int(dados.get('num_portas'))
        recriar_portas = (porta_inicial_atual != porta_inicial_nova) or (num_portas_atual != num_portas_nova)
        pp['nome'] = dados.get('nome')
        pp['andar'] = dados.get('andar')
        pp['porta_inicial'] = porta_inicial_nova
        pp['num_portas'] = num_portas_nova
        pp['status'] = dados.get('status', 'ativo')
        pp['descricao'] = dados.get('descricao')
        _json_write_table(db_file, 'patch_panels', pps)
        if recriar_portas:
            portas = _json_read_table(db_file, 'patch_panel_portas')
            if any(p.get('patch_panel_id') == id and p.get('equipamento_id') is not None for p in portas):
                return jsonify({'erro': 'Não é possível alterar a numeração. Existem conexões ativas.'}), 400
            ...
            _json_write_table(db_file, 'patch_panel_portas', portas)
        detalhes = f"Patch Panel atualizado: {dados.get('nome')}"
        if recriar_portas:
            detalhes += f" - Portas recriadas: {porta_inicial_nova} a {porta_inicial_nova + num_portas_nova - 1}"
        registrar_log(session.get('username', 'desconhecido'), 'ATUALIZAR_PATCH_PANEL', detalhes, 'sucesso', db_file)
        ...
        try:
            ...
            registrar_log(session.get('username', 'desconhecido'), 'ATUALIZAR_PATCH_PANEL', detalhes, 'erro', db_file)
...
def excluir_patch_panel(id):
    ...
    if _is_json_mode(db_file):
        ...
        portas = _json_read_table(db_file, 'patch_panel_portas')
        if any(p.get('patch_panel_id') == id and p.get('equipamento_id') is not None for p in portas):
            return jsonify({'mensagem': 'Não é possível excluir o patch panel. Existem conexões ativas.'}), 400
        portas = [p for p in portas if p.get('patch_panel_id') != id]
        _json_write_table(db_file, 'patch_panel_portas', portas)
        pps = [pp for pp in pps if pp.get('id') != id]
        _json_write_table(db_file, 'patch_panels', pps)
        registrar_log(session.get('username', 'desconhecido'), 'EXCLUIR_PATCH_PANEL', f'Patch Panel ID {id} excluído', 'sucesso', db_file)
...
def atualizar_mapeamento_porta(id):
    ...
    if _is_json_mode(db_file):
        portas = _json_read_table(db_file, 'patch_panel_portas')
        p = next((x for x in portas if x.get('id') == id), None)
        if not p:
        ...
        if not switch_id and not porta_switch:
            ...
            switch_id = None
            porta_switch = None
            status = 'ocupada' if p.get('equipamento_id') else 'livre'
        else:
            if not switch_id or not porta_switch:
                return jsonify({'erro': 'Switch ID e Porta Switch são obrigatórios para mapeamento'}), 400
            status = 'ocupada' if p.get('equipamento_id') else 'mapeada'
        p['switch_id'] = switch_id
        p['porta_switch'] = porta_switch
        p['status'] = status
        _json_write_table(db_file, 'patch_panel_portas', portas)
        # Atualizar status da porta do switch correspondente
        switch_portas = _json_read_table(db_file, 'switch_portas')
        if switch_id and porta_switch:
            sp = next((sp for sp in switch_portas if sp.get('switch_id') == switch_id and int(sp.get('numero_porta') or 0) == int(porta_switch)), None)
            if sp:
                sp['status'] = 'mapeada'
                _json_write_table(db_file, 'switch_portas', switch_portas)
        else:
            # recuperar antigo mapeamento é o próprio p antes da escrita; se existia, liberar a porta do switch se não tiver conexão
            if p.get('switch_id') and p.get('porta_switch'):
                sp = next((sp for sp in switch_portas if sp.get('switch_id') == p.get('switch_id') and int(sp.get('numero_porta') or 0) == int(p.get('porta_switch') or 0)), None)
                if sp:
                    # liberar somente se não tem conexão ativa
                    conexoes = _json_read_table(db_file, 'conexoes')
                    if not any(c.get('porta_id') == sp.get('id') and c.get('status') == 'ativa' for c in conexoes):
                        sp['status'] = 'livre'
                        _json_write_table(db_file, 'switch_portas', switch_portas)
        detalhes = f"Porta {id} mapeada para Switch {switch_id}, Porta {porta_switch}" if switch_id else f"Mapeamento removido da porta {id}"
        registrar_log(session.get('username', 'desconhecido'), 'MAPEAR_PORTA_PATCH_PANEL', detalhes, 'sucesso', db_file)
        ...
        try:
            ...
            registrar_log(session.get('username', 'desconhecido'), 'MAPEAR_PORTA_PATCH_PANEL', detalhes, 'erro', db_file)
...
def listar_portas_patch_panel(id):
    ...
    if _is_json_mode(db_file):
        ...
        switches = {s.get('id'): s for s in _json_read_table(db_file, 'switches')}
        salas = {s.get('id'): s for s in _json_read_table(db_file, 'salas')}
        pp = next((x for x in _json_read_table(db_file, 'patch_panels') if x.get('id') == id), None)
        prefixo_keystone = (pp or {}).get('prefixo_keystone')
        resultado = []
        for p in portas:
            e = equipamentos.get(p.get('equipamento_id')) if p.get('equipamento_id') else None
            resultado.append({
                'id': p.get('id'),
                'numero_porta': p.get('numero_porta'),
                'status': p.get('status'),
                'equipamento_id': p.get('equipamento_id'),
                'switch_id': p.get('switch_id'),
                'porta_switch': p.get('porta_switch'),
                'equipamento_nome': (e or {}).get('nome'),
                'equipamento_tipo': (e or {}).get('tipo'),
                'equipamento_sala_id': (e or {}).get('sala_id'),
                'switch_nome': (switches.get(p.get('switch_id')) or {}).get('nome') if p.get('switch_id') else None,
                'sala_nome': (salas.get((e or {}).get('sala_id')) or {}).get('nome') if (e or {}).get('sala_id') else None,
                'prefixo_keystone': prefixo_keystone
            })
        return jsonify(resultado)
    else:
...
def conectar_equipamento_patch_panel(id):
    ...
    if _is_json_mode(db_file):
        ...
        if not p:
            return jsonify({'erro': 'Porta do patch panel não encontrada'}), 404
        if (p.get('status') or 'livre') == 'ocupada':
            return jsonify({'erro': 'Porta já tem equipamento conectado'}), 400
        equipamentos = _json_read_table(db_file, 'equipamentos')
        e = next((x for x in equipamentos if x.get('id') == equipamento_id), None)
        if not e:
            return jsonify({'erro': 'Equipamento não encontrado'}), 404
        ...
        _json_write_table(db_file, 'patch_panel_portas', portas)
        # atualizar keystone
        pp = next((x for x in _json_read_table(db_file, 'patch_panels') if x.get('id') == p.get('patch_panel_id')), None)
        prefixo = (pp or {}).get('prefixo_keystone') or 'PT'
        numero_porta = int(p.get('numero_porta') or 0)
        e['keystone'] = f"{prefixo}-{numero_porta:04d}"
        _json_write_table(db_file, 'equipamentos', equipamentos)
        detalhes = f"Equipamento {e.get('nome')} conectado à porta {id} do patch panel"
        registrar_log(session.get('username', 'desconhecido'), 'CONECTAR_EQUIPAMENTO_PATCH_PANEL', detalhes, 'sucesso', db_file)
        ...
        try:
            ...
            registrar_log(session.get('username', 'desconhecido'), 'CONECTAR_EQUIPAMENTO_PATCH_PANEL', detalhes, 'erro', db_file)
...
def desconectar_equipamento_patch_panel(id):
    ...
    if _is_json_mode(db_file):
        ...
        equipamentos = _json_read_table(db_file, 'equipamentos')
        e = next((x for x in equipamentos if x.get('id') == p.get('equipamento_id')), None)
        equipamento_nome = (e or {}).get('nome') or 'Desconhecido'
        # novo status depende do mapeamento
        novo_status = 'mapeada' if p.get('switch_id') else 'livre'
        p['equipamento_id'] = None
        ...
        registrar_log(session.get('username', 'desconhecido'), 'DESCONECTAR_EQUIPAMENTO_PATCH_PANEL', detalhes, 'sucesso', db_file)
        ...
        try:
            ...
            registrar_log(session.get('username', 'desconhecido'), 'DESCONECTAR_EQUIPAMENTO_PATCH_PANEL', detalhes, 'erro', db_file)
